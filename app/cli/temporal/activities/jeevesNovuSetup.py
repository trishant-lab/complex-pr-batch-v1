...
class NovuSetup:
    ...
    def get_environment_api_key(self: "NovuSetup", token: str) -> str:
        """
        Get novu env id
        """
        url = f"{self.config.jeeves.novu_url}/v1/environments"
        ...
        api_keys: list = [env.get("apiKeys")[0].get("key") for env in response.json()["data"] if
                    env.get("name") == "Development"]
    ...
    def setup_novu(self: "NovuSetup") -> None:
        ...
        api_keys = self.get_environment_api_key(token=organization_token)
