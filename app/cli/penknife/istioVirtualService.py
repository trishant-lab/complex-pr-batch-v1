...
class IstioVirtualService(K8sResourceBaseClass):
            ...
            "route": [
                {
                    ...
                    "headers": {
                        "response": {"remove": ["x-envoy-upstream-service-time"]}
                    },
                }
            ],
            "match": [
    ...
    def put(self: "IstioVirtualService") -> None:
        ...
        log_info(
            f"VirtualService {ProductName}-vs created in namespace {self.penknife.tenant}"
        )
    ...
    def delete(self: "IstioVirtualService") -> None:
        ...
        try:
            ...
            logger.error(
                f"VirtualService {ProductName}-vs not found in namespace {self.penknife.tenant}"
            )


class IstioCareersVirtualService(K8sResourceBaseClass):
    """
    Namespace class
    """

    def __init__(self: "IstioCareersVirtualService", penknife: PenknifeSpec) -> None:
        """
        Constructor for IstioVirtualService
        """
        self.penknife: PenknifeSpec = penknife
    ...
    def payload(self: "IstioCareersVirtualService") -> dict:
        """
        Payload
        """
        http_list = []
        ...
        body = {
            ...
            "kind": "VirtualService",
            "metadata": {
                "name": "penknife-careers-vs",
                "namespace": self.penknife.tenant,
            },
            "spec": {
                "hosts": [
                    f"{self.penknife.tenant}-careers.{self.penknife_config.domain_name}"
                ],
                "gateways": ["istio-system/istiogateway"],
                "http": http_list,
            },
    ...
    def put(self: "IstioCareersVirtualService") -> None:
        """
        Put VirtualService
        """
        self.k8s_dynamic_client.server_side_apply(
        ...
        log_info(
            f"VirtualService {ProductName}-careers-vs created in namespace {self.penknife.tenant}"
        )

    def delete(self: "IstioCareersVirtualService") -> None:
        """
        Delete VirtualService
        """
        try:
            self.k8s_dynamic_client.delete(
                resource=self.resource,
                name=f"{ProductName}-careers-vs",
                namespace=self.penknife.tenant,
            )
        except NotFoundError:
            logger.error(
                f"VirtualService {ProductName}-careers-vs not found in namespace {self.penknife.tenant}"
            )
