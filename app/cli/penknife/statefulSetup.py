...
def create_product_namespace(penknife: PenknifeSpec, k8s_dynamic_client: DynamicClient) -> None:
    """
    Create namespace in redis
    """
    redis_host = CACHE_HOST.format(tenant=penknife.tenant)
    redis_port = 6379
    ...
    secret = base64.b64decode(
        k8s_dynamic_client.get(resource, namespace=penknife.tenant, name="cache-secret").data.get("REDIS_PASSWORD")
    ).decode()
...
def delete_product_namespace(penknife: PenknifeSpec, k8s_dynamic_client: DynamicClient) -> None:
    """
...
class RedisService(K8sResourceBaseClass):
    ...
    def __init__(self: "RedisService", penknife: PenknifeSpec) -> None:
        """
        Constructor
        """
...
class StateFullSet(K8sResourceBaseClass):
    ...
    def __init__(self: "StateFullSet", penknife: PenknifeSpec) -> None:
        """
        Constructor
        """
        self.penknife: PenknifeSpec = penknife
    ...
    def payload(self: "StateFullSet") -> dict:
        ...
        body = V1StatefulSet(
            ...
            metadata=V1ObjectMeta(namespace=self.penknife.tenant, name=self.name, labels={"app": self.name}),
    ...
    async def put(self: "StateFullSet") -> None:
        ...
        RedisService(self.penknife).put()
        await asyncio.sleep(30)
        create_product_namespace(self.penknife, self.k8s_dynamic_client)
    ...
    def delete(self: "StateFullSet") -> None:
        ...
        """
        delete_product_namespace(self.penknife, self.k8s_dynamic_client)
