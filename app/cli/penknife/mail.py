...
def send_customer_password_mail(penknife: PenknifeSpec, password: str) -> None:
    """
...
def reset_keycloak_user_password(penknife: PenknifeSpec, password: str) -> str:
    ...
    users = keycloak_admin_client.get_users(query={"email": penknife.email}, realm_name=realm_name)

    if len(users) != 1:
        if len(users) == 0:
            msg = f"OnboardingError: User {penknife.email} not found in realm {realm_name}"
        else:
            msg = f"OnboardingError: Found {len(users)} users with email " f"{penknife.email} in realm {realm_name}"
        raise Exception(msg)
...
def onboard_success(penknife: PenknifeSpec) -> None:
    """
    @return:
    """
    password = generate_password(10)
    reset_keycloak_user_password(penknife=penknife, password=password)
    send_customer_password_mail(penknife=penknife, password=password)

    log_info(f"Tenant temporary credentials were sent {penknife.email}")
    return
