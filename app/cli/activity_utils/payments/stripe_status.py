...
async def get_stripe_failure_status(stripe_customer_id: str, invoice_id: str) -> str | None:
    """
    @param invoice_id:
    @return:
    """
    pi = await stripe.PaymentIntent.search_async(query=f'metadata["lago_invoice_id"]:"{invoice_id}"')
    if pi.data:
        return _read_decline_reason(pi.data[0])

    pi = await stripe.PaymentIntent.list_async(customer=stripe_customer_id)
    if pi.data:
