...
async def get_huggingface_endpoint_url(
    ...
    endpoint = get_inference_endpoint(
        endpoint_name,
        token=config.dexit.ai_config.hf_token_write,
        namespace=config.dexit.ai_config.hf_username
    )
    ...
    if endpoint.status in ('running', 'scaledToZero'):
        ...
        while endpoint.status != 'running':
            ...
            endpoint = get_inference_endpoint(
                endpoint_name,
                token=config.dexit.ai_config.hf_token_read,
                namespace=config.dexit.ai_config.hf_username
            )
        logger.info(f'HuggingFace inference endpoint {endpoint_name} is in running state.')
        return endpoint.url
    ...
    async def deploy(self: "HFInferenceEndpointSetup") -> None:
        ...
        for endpoint in endpoints:
            ...
            payload = {
                ...
                'provider': {
                    'region': endpoint.compute_engine.region,
                    'vendor': endpoint.compute_engine.vendor
                },
                'type': 'protected',
                'model': {
                    'framework': 'custom',
