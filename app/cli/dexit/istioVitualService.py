...
class IstioVirtualService(K8sResourceBaseClass):
    ...
    def __init__(self: "IstioVirtualService", dexit: DexitSpec) -> None:
        """
        Constructor
        """
    ...
    def payload(self: "IstioVirtualService") -> dict:
        """
        Returns payload for Istio VirtualService
        """
        http_list = []
        ...
        body = {
            ...
            "spec": {
                "hosts": [f"{self.dexit.tenant}.{self.dexit_config.domain_name}"],
    ...
    def delete(self: "IstioVirtualService") -> None:
        ...
        try:
            self.k8s_dynamic_client.delete(
                resource=self.resource, name=f"{ProductName}-vs", namespace=self.dexit.tenant
            )
