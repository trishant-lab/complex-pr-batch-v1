...
async def create_user_activity(activity_input: CreateDatabaseUserActivityInput) -> None:
    ...
    try:
        ...
        logger.info(f"User {activity_input.username} exists: {response}")
    ...
    if response:
        ...
        await db.execute(
            "PostgresDatabaseSetup/updateUserInDatabase.sql",
            **parameters
        )
        return
...
async def create_schema_activity(activity_input: CreateDatabaseSchemaActivityInput) -> None:
    ...
    try:
        ...
        parameters = {
            ...
            "schema_name": activity_input.schema_name,
...
async def grant_permissions_activity(activity_input: GrantPermissionsActivityInput) -> None:
    ...
    await grant_user_to_all_privileges_on_schema(db, activity_input.db_username, activity_input.schema_name)
