"""Hardening suite 05 — context / retry / redact smoke tests."""
from __future__ import annotations

import pytest


def test_build_context_suite_05():
    from app.cli.temporal.hardening.context import build_context, validate_context

    ctx = build_context("activity_5", tenant="acme-5", attempt=5)
    assert ctx["activity"] == "activity_5"
    assert ctx["tenant"] == "acme-5"
    assert validate_context(ctx) == []


def test_retry_policy_suite_05():
