"""Hardening suite 08 — context / retry / redact smoke tests."""
from __future__ import annotations

import pytest


def test_build_context_suite_08():
    from app.cli.temporal.hardening.context import build_context, validate_context

    ctx = build_context("activity_8", tenant="acme-8", attempt=8)
    assert ctx["activity"] == "activity_8"
    assert ctx["tenant"] == "acme-8"
    assert validate_context(ctx) == []


def test_retry_policy_suite_08():
