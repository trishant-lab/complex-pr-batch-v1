"""Hardening suite 15 — context / retry / redact smoke tests."""
from __future__ import annotations

import pytest


def test_build_context_suite_15():
    from app.cli.temporal.hardening.context import build_context, validate_context

    ctx = build_context("activity_15", tenant="acme-15", attempt=15)
    assert ctx["activity"] == "activity_15"
    assert ctx["tenant"] == "acme-15"
    assert validate_context(ctx) == []


def test_retry_policy_suite_15():
