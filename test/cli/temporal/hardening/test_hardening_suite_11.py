"""Hardening suite 11 — context / retry / redact smoke tests."""
from __future__ import annotations

import pytest


def test_build_context_suite_11():
    from app.cli.temporal.hardening.context import build_context, validate_context

    ctx = build_context("activity_11", tenant="acme-11", attempt=11)
    assert ctx["activity"] == "activity_11"
    assert ctx["tenant"] == "acme-11"
    assert validate_context(ctx) == []


def test_retry_policy_suite_11():
