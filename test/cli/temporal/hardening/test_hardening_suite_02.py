"""Hardening suite 02 — context / retry / redact smoke tests."""
from __future__ import annotations

import pytest


def test_build_context_suite_02():
    from app.cli.temporal.hardening.context import build_context, validate_context

    ctx = build_context("activity_2", tenant="acme-2", attempt=2)
    assert ctx["activity"] == "activity_2"
    assert ctx["tenant"] == "acme-2"
    assert validate_context(ctx) == []


def test_retry_policy_suite_02():
