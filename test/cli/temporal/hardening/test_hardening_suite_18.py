"""Hardening suite 18 — context / retry / redact smoke tests."""
from __future__ import annotations

import pytest


def test_build_context_suite_18():
    from app.cli.temporal.hardening.context import build_context, validate_context

    ctx = build_context("activity_18", tenant="acme-18", attempt=18)
    assert ctx["activity"] == "activity_18"
    assert ctx["tenant"] == "acme-18"
    assert validate_context(ctx) == []


def test_retry_policy_suite_18():
