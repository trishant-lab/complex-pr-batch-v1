"""Hardening suite 01 — context / retry / redact smoke tests."""
from __future__ import annotations

import pytest


def test_build_context_suite_01():
    from app.cli.temporal.hardening.context import build_context, validate_context

    ctx = build_context("activity_1", tenant="acme-1", attempt=1)
    assert ctx["activity"] == "activity_1"
    assert ctx["tenant"] == "acme-1"
    assert validate_context(ctx) == []


def test_retry_policy_suite_01():
