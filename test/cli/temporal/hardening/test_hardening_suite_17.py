"""Hardening suite 17 — context / retry / redact smoke tests."""
from __future__ import annotations

import pytest


def test_build_context_suite_17():
    from app.cli.temporal.hardening.context import build_context, validate_context

    ctx = build_context("activity_17", tenant="acme-17", attempt=17)
    assert ctx["activity"] == "activity_17"
    assert ctx["tenant"] == "acme-17"
    assert validate_context(ctx) == []


def test_retry_policy_suite_17():
