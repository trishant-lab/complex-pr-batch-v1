"""Hardening suite 09 — context / retry / redact smoke tests."""
from __future__ import annotations

import pytest


def test_build_context_suite_09():
    from app.cli.temporal.hardening.context import build_context, validate_context

    ctx = build_context("activity_9", tenant="acme-9", attempt=9)
    assert ctx["activity"] == "activity_9"
    assert ctx["tenant"] == "acme-9"
    assert validate_context(ctx) == []


def test_retry_policy_suite_09():
