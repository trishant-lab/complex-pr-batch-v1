"""Hardening suite 07 — context / retry / redact smoke tests."""
from __future__ import annotations

import pytest


def test_build_context_suite_07():
    from app.cli.temporal.hardening.context import build_context, validate_context

    ctx = build_context("activity_7", tenant="acme-7", attempt=7)
    assert ctx["activity"] == "activity_7"
    assert ctx["tenant"] == "acme-7"
    assert validate_context(ctx) == []


def test_retry_policy_suite_07():
