"""Hardening suite 13 — context / retry / redact smoke tests."""
from __future__ import annotations

import pytest


def test_build_context_suite_13():
    from app.cli.temporal.hardening.context import build_context, validate_context

    ctx = build_context("activity_13", tenant="acme-13", attempt=13)
    assert ctx["activity"] == "activity_13"
    assert ctx["tenant"] == "acme-13"
    assert validate_context(ctx) == []


def test_retry_policy_suite_13():
