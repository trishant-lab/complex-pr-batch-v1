"""Hardening suite 23 — context / retry / redact smoke tests."""
from __future__ import annotations

import pytest


def test_build_context_suite_23():
    from app.cli.temporal.hardening.context import build_context, validate_context

    ctx = build_context("activity_23", tenant="acme-23", attempt=23)
    assert ctx["activity"] == "activity_23"
    assert ctx["tenant"] == "acme-23"
    assert validate_context(ctx) == []


def test_retry_policy_suite_23():
