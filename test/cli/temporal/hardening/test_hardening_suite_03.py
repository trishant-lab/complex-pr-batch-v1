"""Hardening suite 03 — context / retry / redact smoke tests."""
from __future__ import annotations

import pytest


def test_build_context_suite_03():
    from app.cli.temporal.hardening.context import build_context, validate_context

    ctx = build_context("activity_3", tenant="acme-3", attempt=3)
    assert ctx["activity"] == "activity_3"
    assert ctx["tenant"] == "acme-3"
    assert validate_context(ctx) == []


def test_retry_policy_suite_03():
