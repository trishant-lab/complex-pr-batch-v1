"""Hardening suite 21 — context / retry / redact smoke tests."""
from __future__ import annotations

import pytest


def test_build_context_suite_21():
    from app.cli.temporal.hardening.context import build_context, validate_context

    ctx = build_context("activity_21", tenant="acme-21", attempt=21)
    assert ctx["activity"] == "activity_21"
    assert ctx["tenant"] == "acme-21"
    assert validate_context(ctx) == []


def test_retry_policy_suite_21():
